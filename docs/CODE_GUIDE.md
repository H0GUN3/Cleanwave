...
- 오분류 시 문제, 선택지, 해설을 포함한 퀴즈형 피드백 팝업을 표시한다.
- 퀴즈형 피드백 팝업은 오분류 직후에만 표시하며, 별도 튜토리얼이나 상시 퀴즈 모드로 확장하지 않는다.
- 퀴즈 문항 후보는 `docs/QUIZ_BANK.md`를 기준으로 한다.
- HUD는 TextMeshPro 텍스트 중심으로 구성한다.
- 퀴즈형 피드백 팝업의 문제, 선택지, 해설도 TextMeshPro 텍스트로 출력한다.
- 팝업 구조는 `QuizFeedbackPanel`, `DimBackground`, `PanelBackground`, `QuestionIcon`, `QuestionText`, `WrongTrashIcon`, `ChoiceButtonA/B`, `ExplanationBox`, `CorrectBinIcon`, `ConfirmButton`을 기준으로 한다.
- 플레이어가 A/B 중 하나를 선택하면 정답 여부와 관계없이 올바른 수거함과 해설을 보여주고, `확인` 입력 후 플레이로 복귀한다.
- 가방 슬롯은 선택된 슬롯을 강조한다.
...
- [ ] `F`로 선택 쓰레기를 버린다.
- [ ] 잘못 버리면 점수 `-5`가 적용된다.
- [ ] 잘못 버리면 오분류 퀴즈형 피드백 팝업이 1회 표시된다.
- [ ] 퀴즈형 피드백 팝업에서 선택지 선택 후 올바른 수거함과 해설이 표시된다.
